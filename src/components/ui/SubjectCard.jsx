...
// 学科卡片：图标 + 名称 + 描述 + 学习进度 + 进入按钮
export default function SubjectCard({ subject, onEnter }) {
  const { name, colorVar, icon, desc, progress, lessons } = subject
  ...
  return (
    <article className={styles.card} style={{ '--accent': color }}>
      ...
      <ProgressBar value={progress} color={color} label="学习进度" />
